...
if (!in_array($status, $valid_statuses, true)) {
    $response->error('Invalid status. Allowed values: ' . implode(', ', $valid_statuses), 400);
    exit;
}
...
try {
    ...
    if ($stmt->rowCount() > 0) {
        $response->success(['id' => $id, 'status' => $status], 'Message status updated successfully', 200);
        exit;
    } else {
        // Could be "not found" or "status already set"
        $existsStmt = $pdo->prepare("SELECT 1 FROM contact_messages WHERE id = :id");
