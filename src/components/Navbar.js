...
//       menuItems: defaultNavData.menuItems || [],
...
//                             navigate(item.path); // ✅ replaced window.location.href
//                           }
//                         }
//                       }}
//                     >
...
const Navbar = () => {
  ...
  const closeAllDropdowns = () => {
    if (navData && navData.menuItems) {
      ...
      navData.menuItems.forEach(item => {
        if (item.dropdown) {
          reset[item.megaMenu] = false;
          reset[item.dropdown] = false;
        }
      });
      setDropdownStates(reset);
    }
  };
  ...
  // Close menu & dropdowns on route change (mobile + general safety)
  useEffect(() => {
    setIsOpen(false);
    closeAllDropdowns();
  }, [location]); // navData no longer needed here
  ...
  const renderMegaMenu = (item) => {
    // Do not show the mega menu while on the Services page itself
    const shouldShow = dropdownStates[item.id] && location.pathname !== item.path;
    return (
      <div className={`mega-menu ${shouldShow ? 'show' : ''}`}>
        <div className="mega-menu-container">
          {item.categories.map(category => (
            <div key={category.id} className="mega-menu-column">
              <h3 className="mega-menu-title">{category.title}</h3>
          ...
          >
            {navData.menuItems.map((item) => (
              <li
                ...
                className={`nav-item ${item.dropdown ? 'dropdown' : ''} ${item.megaMenu ? 'mega-dropdown' : ''}`}
                ref={el => item.dropdown && (dropdownRefs.current[item.id] = el)}
                onMouseEnter={() => item.dropdown && location.pathname !== item.path && handleMouseEnter(item.id)}
                onMouseLeave={() => item.dropdown && handleMouseLeave(item.id)}
              >
                {item.dropdown ? (
                  <>
                    <Link
                      ...
                      style={{ color: navData.navbarTextColor }}
                      onClick={(e) => {
                        if (item.megaMenu && window.innerWidth > 960) {
                          // Prevent default so we can cleanly close menus before navigating
                          e.preventDefault();
                          navigate(item.path); // navigate without using window
                          // closeAllDropdowns();
                          // setIsOpen(false);
                          // item.megaMenu = false;
                        }
                        // On mobile, Link will navigate normally; dropdowns close on route change effect
                      }}
                    >
                      {item.text}
                    </Link>
